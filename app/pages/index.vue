<template>
  <v-container fluid>
    <v-text-field label="Übersetzung suchen" hide-details color="primary"></v-text-field>
    <v-checkbox label="Nur Fehlende Übersetzungen anzeigen" hide-details density="compact" color="primary"></v-checkbox>
    <v-treeview v-model="selected" :hide-actions="false" :indent-lines="true" :items="items" :separate-roots="true" density="compact" item-value="id" open-all
      open-on-click return-object>
      <template v-slot:prepend="{ item, isOpen }">
        <v-icon :icon="getIcon(item, isOpen)" :color="isLocaleMissing(item) ? 'error' : undefined"></v-icon>
      </template>
        ...
        <div>
          ...
          <template v-if="item.value">
            <template v-for="locale of locales" :key="locale">
              <v-chip density="compact" :color="item.value[locale] ? 'success' : 'error'" class="mx-1">{{ locale }}</v-chip>
            </template>
          </template>
          <!-- {{ item.value }} -->
        </div>
      ...
      <template v-slot:append="{ item, isOpen }">
        <template v-if="item.key">
          <span>{{ item.key }}</span>
          <v-btn icon="mdi-content-copy" size="small" class="ml-2" @click.stop></v-btn>
        </template>
      </template>
    </v-treeview>
  </v-container>
  <v-navigation-drawer location="end" width="400">
    <div v-if="selected" class="pa-3">
      <div class="text-center">
        <v-icon :icon="getIcon(selected[0], false)" size="80"></v-icon>
      </div>
      ...
      <div v-for="[locale, value] of Object.entries(selected[0].value)" :key="locale" class="py-2">
        <v-chip density="compact" :color="value ? 'default' : 'error'" class="mx-1">{{ locale }}</v-chip>
        {{ value }}
      </div>
    </div>
  </v-navigation-drawer>
...
function getIcon(item: TreeViewEntry, isOpen: boolean) {
  ...
  if (isLocaleMissing(item)) return 'mdi-alert'
  return 'mdi-earth'
}
